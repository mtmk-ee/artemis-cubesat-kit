...
void TestSensor(int bus, int addr, int duration = 5) {
	...
	printf("Mask Exponent:      %d (", config.MaskExponent); PrintBinary(config.MaskExponent, 2); printf(")");
	printf("Polarity:           %d (", config.Polarity); PrintBinary(config.Polarity, 2); printf(")");
	printf("Latch:              %d (", config.Latch); PrintBinary(config.Latch, 2); printf(")");
	printf("Flag Low:           %d (", config.FlagLow); PrintBinary(config.FlagLow, 2); printf(")");
	printf("Flag High:          %d (", config.FlagHigh); PrintBinary(config.FlagHigh, 2); printf(")");
	printf("Conversion Ready:   %d (", config.ConversionReady); PrintBinary(config.ConversionReady, 2); printf(")");
	printf("Overflow Flag:      %d (", config.OverflowFlag); PrintBinary(config.OverflowFlag, 2); printf(")");
	printf("Mode of Conversion: %d (", config.ModeOfConversionOperation); PrintBinary(config.ModeOfConversionOperation, 2); printf(")");
	printf("Conversion Time:    %d (", config.ConversionTime); PrintBinary(config.ConversionTime, 2); printf(")");
	printf("Range Number:       %d (", config.RangeNumber); PrintBinary(config.RangeNumber, 2); printf(")");
	printf("===========================================\n\n");
